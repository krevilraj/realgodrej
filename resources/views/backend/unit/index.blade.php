    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>All Business Units
                            <small>({{ $unitMembersCount }})</small>
                        </h1>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Business Unit</h3>
                                <a href="{{ route('dashboard.unit.create') }}" class="btn btn-sm btn-danger pull-right">Add
                                    New</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
    ...
    <script>
        $(document).ready(function () {
            $('.datatable').DataTable({
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            var unitEditUrl = "{{ route('dashboard.unit.edit', ':id') }}";

                            unitEditUrl = unitEditUrl.replace(':id', data);
                            return '<a href="' + unitEditUrl + '">#' + data + '</a>';
                        }
                    },
                    {
                        data: 'title',
                        render: function (data, type, row) {
                            return '<a href="{{ url('/dashboard/unit') }}' + '/' + row.id + '/edit' + '">' + data + '</a>';
                        }
                    },
                    {
                        data: 'status',
                    ...
                    {data: 'created_at'},
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row) {
                            var tempEditUrl = "{{ route('dashboard.unit.edit', ':id') }}";
                            var tempDeleteUrl = "{{ route('dashboard.unit.destroy', ':id') }}";
                ...
                ajax: '{{ route('dashboard.unit.json') }}'
            });
        });
    </script>
@endpush
